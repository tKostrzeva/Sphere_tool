...
const N_REFRACT = 48;      // pre-tinted sprites around the hue wheel (refraction)
...
let refractSprites = [];   // same glow sprite around the full hue wheel (light refraction)
...
let refractOn, refractAmount, refractScale, refractSeed, refractSpread, refractHue, refractIntensity;
...
function buildSprites() {
  ...
  buildRefractSprites();    // …and the refraction hue-wheel sprites
}

// HSV (h,s,v in 0..1) → [r,g,b] 0..255. Used to build the refraction spectrum.
...
// A refracted point just indexes into this by its computed hue — so the Hue /
// Spread sliders cost nothing at draw time (no rebuild). Vivid but not neon.
function buildRefractSprites() {
  const gCore = map(glow, 1, 100, 0.16, 0.5);
  const coreStop = map(pointSize, 1, 100, 0.09, 0.5);
  refractSprites = new Array(N_REFRACT);
  for (let k = 0; k < N_REFRACT; k++) {
    const [r, g, bl] = hsv2rgb(k / N_REFRACT, 0.82, 1.0);
    refractSprites[k] = makeSprite(r, g, bl, [
...
function setup() {
  ...
  bind('refract-toggle', v => refractOn = v, null, true);
  bind('refract-amount-slider', v => refractAmount = +v);
  bind('refract-scale-slider', v => refractScale = +v);
  bind('refract-seed-slider', v => refractSeed = +v);
  bind('refract-spread-slider', v => refractSpread = +v);
  bind('refract-hue-slider', v => refractHue = +v);
  bind('refract-intensity-slider', v => refractIntensity = +v);
...
function renderScene(ctx, W, H, opaque, scale, stride) {
  ...
  // Light-refraction params (computed once per frame).
  const rCut = 1 - refractAmount / 100;               // noise threshold: higher amount = more points
  const rFreq = map(refractScale, 1, 100, 0.4, 6.0);  // spatial scale of the refraction patches
  const rOff = refractSeed * 0.137 + 30.0;            // decorrelate the selection from the shape noise
  const rHueF = refractHue / 100;                     // spectrum centre (0..1 around the hue wheel)
  const rSpread = refractSpread / 100 * 1.5;          // how much of the wheel the surface noise spans
  const rInt = refractIntensity / 100;                // brightness / size boost of a refracted point
  const gapWorld = minDim * map(membraneGap, 0, 100, 0.0, 0.05);
  ...
  if (renderMode === 'points') {
    for (let i = 0; i < sphereCount; i += stride) {
      ...
      // Base colour = the A→B bucket by the shape noise (one merged sprite).
      let sprite = coreSprites[Math.min(N_BUCKETS - 1, Math.max(0, Math.round(n * (N_BUCKETS - 1))))];
      const persp = focal / (focal - rz2 * rBase);
      const sx = cx + rx * rBase * persp, sy = cy + ry * rBase * persp;
      let gs = glowSize * persp;
      let a = alpha;

      // Light refraction: a noise-selected subset of the SAME points is recoloured
      // across the spectrum (hue driven by the shape noise → smooth dispersion) and
      // lit a little brighter, so it reads as caught, refracted light.
      if (refractOn) {
        const rnv = noise(d.x * rFreq + rOff, d.y * rFreq + 12.7, d.z * rFreq + noiseT);
        if (rnv > rCut) {
          let hf = rHueF + (n - 0.5) * rSpread;
          hf -= Math.floor(hf);                              // wrap to [0,1)
          sprite = refractSprites[Math.min(N_REFRACT - 1, Math.floor(hf * N_REFRACT))];
          gs *= 1 + 0.9 * rInt;
          a = Math.min(1, alpha * (1 + 1.4 * rInt));
        }
      }
      ctx.globalAlpha = a;
      ctx.drawImage(sprite, sx - gs / 2, sy - gs / 2, gs, gs);
    }
